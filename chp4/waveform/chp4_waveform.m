...
function [trained_dnn, train_info] = trainDNN (gm, patch_dur)
    trainSamples = load("chp4/waveform/cache/dnnTrainGwnWaveform.mat");
    ...
    % training_option.MiniBatchSize = 38;
    % training_option.InitialLearnRate = 0.05;
    % [trained_dnn, train_info] = trainNetwork(XTrain, YTrain, trained_dnn.Layers, training_option);
end

function [cumu_resp] = checkResponse (gm, patch_dur, dataset_path)
    test_data = load(dataset_path);
    sample_size = length(test_data.testY);
    assert(gm.netTestParam.max_iter == sample_size);

    cumu_resp = zeros(gm.netTestParam.max_iter, prod(gm.topo_space));
    ...
    for i = 1:gm.netTestParam.max_iter
        frmL = test_data.testX{i}{1};
        frmR = test_data.testX{i}{2};
        loc_idx = test_data.testY{i};

        startPt = 0:patch_stride:size(frmL, 1)-patch_len;
        for j = 1:length(startPt)
            chkL = frmL(j+(1:patch_len),:);
            chkR = frmR(j+(1:patch_len),:);
    ...
    for i = 1:19
        figure;
        imagesc(reshape(cumu_resp(i,:), gm.topo_space(1), gm.topo_space(2)));
    end
end
...
function [mae, chunk_mae, azimuth_predicts, azimuth_truths, cumu_resp] = testModel (...
    ...
    testSamples = load("chp4/waveform/cache/dnnTestGwnWaveform.mat");
...
function visualizeMap (gm)
    ...
    uy = max(gm.gsm{1}.bases{1}, [], "all");
    ly = min(gm.gsm{1}.bases{1}, [], "all");

    figure;
    for i=1:r*c
        subplot(r,c,i);
        [ir, ic] = ind2sub([r c], i);
        b = gm.gsm{1}.bases{1}(:,m(ir,ic));
        l = size(b, 1) / 2;
        plot(1:1:l, b(1:l,:));
        hold on;
    ...
    uy = max(gm.gsm{1}.bases{2}, [], "all");
    ly = min(gm.gsm{1}.bases{2}, [], "all");

    figure;
    for i=1:r*c
        subplot(r,c,i);
        [ir, ic] = ind2sub([r c], i);
        b = gm.gsm{1}.bases{2}(:,m(ir,ic));
        l = size(b, 1) / 2;
        plot(1:1:l, b(1:l,:));
        hold on;
        plot(l+1:1:l*2, b(l+1:end,:), "Color", "r");
...
%%% train gassom
% gm = initGassom([map_size, map_size], 50000, patch_dur);
% gm = initGassom([5, 5], 24000);
% gm = loadGassom([map_size, map_size], 50000, save_folder + "gsm.mat", patch_dur);
% gm.trainGASSOM_timit3();
% gsm = gm.gsm{1};
% save(save_folder + "gsm.mat", "gsm");
% visualizeMap(gm);

% cumu_resp = checkResponse(gm, patch_dur, "cache/dnnTestGwnWF_kemar_0_1900_bandpass_125_500.mat");

%%% calculate BMT
% norm_winners = load("chp4_result/10x10_sz5_sf1/norm_winners.mat").norm_winners;
% norm_winners = (winners.*100)./(gm.max_iter * batch_size);
